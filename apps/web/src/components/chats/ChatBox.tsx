            ...
            return (
              <div key={chat.id} className={`flex ${isUser ? "justify-end" : "items-start gap-4"}`}>
                ...
                <div className={`${isUser ? "text-right" : ""} max-w-[80%]`}>
                  ...
                  <div className="mt-2 text-gray-500 text-theme-xs dark:text-gray-400">
                    {isUser
                      ? formatRelative(chat.createdAt)
                      : `KitchenDocs AI - ${formatRelative(chat.createdAt)}`}
                    {!isUser && citations.length > 0 && (
                      <div
                        ref={isOpen ? citationAnchorRef : undefined}
                        className="relative mt-3 inline-block ml-4"
                      >
                        <button
                          type="button"
                          onClick={() => setActiveCitationId(isOpen ? null : chat.id)}
                          className="inline-flex items-center gap-2 text-xs font-medium text-brand-600 hover:text-brand-700 dark:text-brand-400 dark:hover:text-brand-300"
                        ...
                        {isOpen && (
                          <div
                            ref={citationRef}
                            className={`absolute ${citationPositionClass} z-30 w-[320px] rounded-2xl border border-gray-200 bg-white/95 p-3 shadow-lg backdrop-blur-sm dark:border-gray-800 dark:bg-gray-900/95`}
                          >
                            ...
                            <div className="space-y-3">
                              {citations.map((item, index) => {
                                const meta = item.metadata || {};
                                const title = buildCitationTitle(meta);
                                const fileId = getCitationFileId(meta);
                                const externalBaseUrl =
                                  getCitationUrl(meta) || (fileId ? fileUrlById[fileId] : null);
                                const previewBaseUrl = buildPreviewUrl(fileId) || externalBaseUrl;
                                const pageValue = Number(meta.page);
                                const pageNumber = Number.isFinite(pageValue)
                                  ? pageValue
                                  : undefined;
                                const link = buildCitationLink(
                                  externalBaseUrl || previewBaseUrl,
                                  pageNumber,
                                );
                                const page = meta.page ? `Page ${meta.page}` : null;
                                const sourceLine =
                                  meta.source && meta.source !== title ? meta.source : null;

                                return (
                                  <div
                                    key={`${chat.id}-cite-${index}`}
                                    className="rounded-xl border border-gray-200/70 p-3 text-sm text-gray-700 dark:border-gray-800 dark:text-gray-300"
                                  >
